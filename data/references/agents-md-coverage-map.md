...
```json
{
  ...
  "statements": [
    ...
    {"id": "coord.specialist-no-mcp", "source_section": "Agent Inventory > GitLab-note (L57) + GitLab-MCP-Tools Reference (L213)", "class": "router", "out_section": "Cross-agent coordination", "anchor": "call `mcp__GitLab__*`", "disposition": "condensed", "text": "- Specialist agents (gitlab-workflow, release-manager, the K8s-ops agents) carry only Read/Glob/Grep/Bash — they CANNOT call `mcp__GitLab__*` or other MCP tools. GitLab and MCP reads and writes happen in the MAIN session or via skills."},
